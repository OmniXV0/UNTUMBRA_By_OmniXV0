...
@onready var playback = enemy_animation_tree.get("parameters/EnemyLordState/playback") as AnimationNodeStateMachinePlayback
...
func enemy_lord_attack():
	AudioController.play_sfx(AudioController.enemy_lord_spawn_sfx)
	
func enemy_laser_mirrored() -> bool:
	print("HERE")
	var mirror = false
	if direction.x < 0:
		mirror = true
...
func change_animation(animation_name: String):
	enemy_animation_tree.get_tree_root().get_node("EnemyLordState").get_node("Attack").animation = animation_name
...
func free_enemy() -> void:
	...
	death_effect.play("LordDeathEffect")
